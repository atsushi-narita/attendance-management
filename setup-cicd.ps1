...
function Deploy-Pipeline {
    ...
    try {
        $stackStatus = aws cloudformation describe-stacks --stack-name "$StackName-$Environment" --region $Region --query "Stacks[0].StackStatus" --output text 2>$null
        if ($LASTEXITCODE -eq 0 -and $stackStatus -eq "ROLLBACK_COMPLETE") {
            Write-InfoMessage "Stack is in ROLLBACK_COMPLETE state. Deleting and recreating..."
            $null = aws cloudformation delete-stack --stack-name "$StackName-$Environment" --region $Region
            Write-InfoMessage "Waiting for stack deletion to complete..."
            $null = aws cloudformation wait stack-delete-complete --stack-name "$StackName-$Environment" --region $Region
            Write-SuccessMessage "Stack deleted successfully"
        }
    }
    catch {
